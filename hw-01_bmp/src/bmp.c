...
size_t calc_bmp_content_size(bmp_size_t size) {
    size_t row_size = size.width * sizeof(rgb_triple_t);
    row_size += (4 - row_size % 4) % 4; // Data alignment
    return row_size * size.height;
}

void **alloc_2d_array(size_t rows, size_t row_size, size_t content_size) {
...
bmp_err_t read_file_header(bmp_t *bmp, FILE *in_file) {
    size_t read_items = fread(&bmp->file_header, sizeof(struct BITMAPFILEHEADER), 1, in_file);
    if (read_items != 1)
        return BMP_ERR_FILE_READ;
    return BMP_OK;
}

bmp_err_t read_info_header(bmp_t *bmp, FILE *in_file) {
    size_t read_items = fread(&bmp->info_header, sizeof(struct BITMAPINFOHEADER), 1, in_file);
    if (read_items != 1)
        return BMP_ERR_FILE_READ;
...
bmp_err_t read_pixel_data(bmp_t *bmp, FILE *in_file) {
...
#define load_bmp_handle_err(bmp_err) \
    if (bmp_err != BMP_OK) { \
        free(bmp); \
        return bmp_err; \
    }
...
bmp_err_t write_file_header(const bmp_t *bmp, FILE *out_file) {
    struct BITMAPFILEHEADER correct_header = bmp->file_header;

    // Writing pixels right after headers
    size_t content_pos = sizeof(struct BITMAPFILEHEADER) + sizeof(struct BITMAPINFOHEADER);
...
bmp_err_t write_info_header(const bmp_t *bmp, FILE *out_file) {
...
bmp_err_t write_pixel_data(const bmp_t *bmp, FILE *out_file) {
...
bmp_err_t create_bmp_with_size(bmp_t **dst, const bmp_t *ref, bmp_size_t size) {
    bmp_t *bmp = malloc(sizeof(bmp_t));
    if (!bmp)
        return BMP_ERR_MEM_ALLOC;
...
bmp_err_t rotate_bmp_clockwise_90(bmp_t **out_dst, const bmp_t *src) {
    *out_dst = NULL;
    bmp_t *dst;
